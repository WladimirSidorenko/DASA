...
\begin{abstract}
  ...
  Sentiment Treebank, getting\todo{add evaluation numbers}.
\end{abstract}
...
its very inception~\cite[pp.~\todo{add pages}]{Liu:12}.  However, most
...
\ie{} \emph{ignoring the structure of discourse}.  Nevertheless, as
already noted by~\newcite{Polanyi:06}, discourse relations and
structure are one of the most influential linguistic factors that
might significantly change the polarity of an opinion.

Indeed, if we look at Figure~\ref{fig:rst-tree}, which shows a
fragment of an IMDB review of the ``House M.D'' series, and simply
compare the number of positive and negative terms in that snippet, we
will tend to think that the polarity of the review is negative, as
most of its polar terms indicate a critical attitude (seven negative
terms versus two positive).  However, the actual opinion of the author
about the series is unequivocally positive, and she only expresses her
general negative general stance in order to stress the strengths of
this show.
...
\todo[inline]{Lemmatization:}
\todo[inline]{Discourse segmentation: \newcite{Wang:18}}
\todo[inline]{Gold label}
